...
import { INFANT_HERO } from '@/lib/data/infant-content';

export function InfantHero() {
  return (
    <section className="relative flex min-h-[500px] w-full items-center justify-center overflow-hidden md:min-h-[600px]">
      <Image
        src="/images/programs/nido-hero.jpg"
        alt="Infant Care at Sundrops Montessori"
        fill
      ...
      <div className="relative z-10 container mx-auto max-w-4xl px-4 py-20 text-center">
        ...
        <p className="font-script mt-3 text-lg tracking-wider text-white md:text-xl">{INFANT_HERO.subtitle}</p>
        <p className="mt-2 text-lg text-white">{INFANT_HERO.ageRange}</p>
        <div className="mt-8">
          <Button asChild size="lg" className="border border-black bg-white text-black hover:bg-white/90">
            <Link href={INFANT_HERO.cta.href}>{INFANT_HERO.cta.label}</Link>
          </Button>
        </div>
      </div>
    </section>
