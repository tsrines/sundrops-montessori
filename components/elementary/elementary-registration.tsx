...
import { ELEMENTARY_REGISTRATION } from '@/lib/data/elementary-content';

const STEP_ICONS = [ClipboardList, FileText, CheckCircle] as const;

export function ElementaryRegistration() {
        ...
        <div className="grid gap-10 md:grid-cols-2 md:gap-16">
          <div className="flex flex-col justify-center">
            <h2 className="mb-2 font-serif text-3xl font-bold tracking-tight text-white md:text-4xl">
              {ELEMENTARY_REGISTRATION.heading}
            </h2>
            <h3 className="mb-4 text-lg font-medium text-gray-300">{ELEMENTARY_REGISTRATION.subheading}</h3>
            <p className="leading-relaxed text-gray-200">{ELEMENTARY_REGISTRATION.description}</p>
          </div>
          <div className="flex flex-col gap-6">
            {ELEMENTARY_REGISTRATION.steps.map((step, index) => {
