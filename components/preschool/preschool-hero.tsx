...
import { PRESCHOOL_HERO } from '@/lib/data/preschool-content';

export function PreschoolHero() {
  return (
    <section className="relative flex min-h-[500px] w-full items-center justify-center overflow-hidden md:min-h-[600px]">
      <Image
        ...
        alt="Preschool and Kindergarten Programs at Sundrops Montessori"
        fill
      ...
      <div className="relative z-10 container mx-auto max-w-4xl px-4 py-20 text-center">
        <h1 className="font-serif text-5xl font-bold tracking-tight text-white md:text-6xl lg:text-7xl">
          {PRESCHOOL_HERO.title}
        </h1>
        <p className="font-script mt-3 text-lg tracking-wider text-white md:text-xl">{PRESCHOOL_HERO.subtitle}</p>
        <p className="mt-2 text-lg text-white">{PRESCHOOL_HERO.ageRange}</p>
        <div className="mt-8">
          <Button asChild size="lg" className="border border-black bg-white text-black hover:bg-white/90">
            <Link href={PRESCHOOL_HERO.cta.href}>{PRESCHOOL_HERO.cta.label}</Link>
          </Button>
        </div>
      </div>
    </section>
